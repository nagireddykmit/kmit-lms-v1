...
$sql="INSERT INTO tblleaves(LeaveType,ToDate,FromDate,Description,Reason,Status,IsRead,empid) VALUES(:leavetype,:todate,:fromdate,:description,:Reason,:status,:isread,:eid)";
...
echo "<script> console.log(".$lastInsertId.");</script>";
    for ($i=0; $i<=9; $i++) 
    {
		...
		$sql1="INSERT INTO classadjustments (leaveid, empid, empname, adjustedfrom, department, Year, Section, Subject, Timings, classdate,adjempid) VALUES (:lastInsertId, :aempid, :aempname, :emname, :adept, :ayear, :asection, :asubject, :aclass, :adate, :empid)";
		...
		$atmt->bindParam(':empid',$empid,PDO::PARAM_STR);
        $atmt->execute();
    }
		...
		.look
{
 ...
 <script>
	...
	{
		if(lt.options[lt.selectedIndex].text=="Half Day(FN)" || lt.options[lt.selectedIndex].text=="Half Day (AN)")
		{
                ...
                $('#adjustments').click(function(event){
                    ...
                    $('#adjust_fields').append('<div id="position'+countAdj+'"> \
								...
								<select name="sel_dept'+countAdj+'">\
									...
									<option value="H and S">H&S</option>\
								</select>\
