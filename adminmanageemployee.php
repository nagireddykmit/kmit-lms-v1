...
$id=$_GET['inid'];
$status=0;
$sql = "update tblemployees set Status=:status , RegDate=current_timestamp() WHERE id=:id";
	  ...
	  <?php if($msg){?><div class="succWrap"><strong>SUCCESS</strong> : <?php echo htmlentities($msg); ?> </div><?php }?>
		<table class="table table-striped table-responsive table-bordered" id="example" cellspacing="0" style="table-layout: auto;width: 100%;" >
			...
			//$empid=$_SESSION['empid'];
			 ...
			 $sql = "SELECT EmpId,FirstName,LastName,Department,Status,RegDate,id from  tblemployees order by Department";
			$query = $dbh -> prepare($sql);
			//$query->bindParam(':empid',$dept,PDO::PARAM_STR);
			$query->execute();
			...
			{               ?>  
					<tr class="item">
						...
						<td style="font: small-caps 15px/30px Georgia, serif;"><!--<a href="editemployee.php?empid=<?php echo htmlentities($result->id);?>"><span class="glyphicon glyphicon-edit icon-update"></span></span></a>-->
					<?php if($result->Status==1)
{?>
<a href="adminmanageemployee.php?inid=<?php echo htmlentities($result->id);?>" onclick="return confirm('Are you sure you want to inactive this Employe?');" > <span class="glyphicon glyphicon-remove icon-invalid"></span>
<?php } else {?>

						<a href="adminmanageemployee.php?id=<?php echo htmlentities($result->id);?>" onclick="return confirm('Are you sure you want to active this employee?');""><span class="glyphicon glyphicon-ok icon-valid"></span>
						<?php } ?> </td>
					</tr>
					 <?php $cnt++;} }?>
				</tbody>
